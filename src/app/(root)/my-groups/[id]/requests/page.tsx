...
const JoinRequestsPage = () => {
  ...
  useEffect(() => {
    const fetchJoinRequests = async () => {
      try {
        setLoading(true);
        const data = await customFetch.get<JoinRequest[]>(
          `/meetings/${meetingId}/requests`
        );
        setRequests(data);
      } catch (err) {
  ...
  const handleApproveReject = async (
    ...
    try {
      ...
      toast.success(
        `요청이 ${action === "approve" ? "승인" : "거절"}되었습니다.`
      );
    } catch (err) {
              ...
              >
                        ...
                        {(() => {
                          const date =
                            typeof request.requestedAt === "string"
                              ...
                              : new Date(request.requestedAt._seconds * 1000);
                ...
                {request.status === "pending" && (
                  <div className="flex gap-2 pt-3 border-t border-gray-100">
                    <button
                      onClick={() =>
                        handleApproveReject(request.requestId, "approve")
                      }
                    ...
                    <button
                      onClick={() =>
                        handleApproveReject(request.requestId, "reject")
                      }
